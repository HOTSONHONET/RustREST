...
//Mode: User struct iwth id, name, email
    ...
    match stream.read(&mut buffer){
        Ok(size) => {
            ...
            let (status_line, content) = match &*request {
                ...
                _ => (NOT_FOUND.to_string(), "NOT_FOUND".to_string()),
            };
...
// handle get all requests function
...
// handle the put request function
fn handle_put_request(request: &str) -> (String, String){
    match(
        ...
        (Ok(id), Ok(user), Ok(mut client)) => {
            ...
            &[&user.name, &user.email, &user.id]).unwrap();
...
// handle the delete request function
